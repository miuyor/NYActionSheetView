...
- (UIView *)createMenuView {
    ...
    for (int i = 0; i < self.ButtonTitlesArray.count; i++) {
        ...
        if (i == self.ButtonTitlesArray.count - 1) {
            ...
            image = [[UIImage imageNamed:@"Cancel_btn.png"] stretchableImageWithLeftCapWidth:5 topCapHeight:5];
        }else {
            image = [[UIImage imageNamed:@"photo_btn.png"] stretchableImageWithLeftCapWidth:5 topCapHeight:5];
...
- (void)clickSheetButton:(UIButton *)sender {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(NYSheetView:clickButtonIndex:)]) {
        
        [_delegate NYSheetView:self clickButtonIndex:sender.tag];
    }
